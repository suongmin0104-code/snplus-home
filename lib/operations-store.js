...
export const ESTIMATE_PDF_PREFIX = `${ROOT_PREFIX}estimate-pdfs/`;
...
export function estimatePdfPath(id) {
  return isOperationId(id) ? `${ESTIMATE_PDF_PREFIX}${id}.pdf` : "";
}
...
export async function saveEstimate(input, actorId = "") {
  ...
  return writeEntry(ESTIMATE_PREFIX, {
    ...
    ...(existing?.documentFile ? { documentFile: existing.documentFile } : {}),
    ...auditFields(existing, actorId)
  });
}

export async function saveEstimatePdf(id, content, filename, actorId = "") {
  if (!isOperationId(id)) throw new Error("OPERATION_ID_INVALID");
  const existing = await readEntry(ESTIMATE_PREFIX, id);
  if (!existing?.document) throw new Error("ESTIMATE_NOT_FOUND");
  const body = Buffer.isBuffer(content) ? content : Buffer.from(content);
  const path = estimatePdfPath(id);
  const name = cleanText(filename, 120).replace(/\.pdf$/i, "") || `견적서_${existing.documentNumber || id}`;
  ...
  await writeEntry(ESTIMATE_PREFIX, entry);
  return { entry, documentFile };
}

export async function readEstimatePdf(id) {
  if (!isOperationId(id)) throw new Error("OPERATION_ID_INVALID");
  const entry = await readEntry(ESTIMATE_PREFIX, id);
  const path = String(entry?.documentFile?.path ?? "");
  if (!entry || path !== estimatePdfPath(id)) return null;
  const result = await get(path, { access: "private", useCache: false });
  if (!result?.stream) return null;
  return { entry, documentFile: entry.documentFile, blob: result.blob, stream: result.stream };
}

export async function deleteEstimate(id) {
  if (!isOperationId(id)) throw new Error("OPERATION_ID_INVALID");
  await del([itemPath(ESTIMATE_PREFIX, id), estimatePdfPath(id)]);
}
...
export function summarizeDeletedInventoryMovements(movements, today = new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Seoul" })) {
  const currentMonth = String(today).slice(0, 7);
  const entries = Array.isArray(movements) ? movements : [];
  return {
    total: entries.length,
    monthIn: entries.filter((movement) => movement.type === "in" && movement.createdAt?.startsWith(currentMonth)).length,
    monthOut: entries.filter((movement) => movement.type === "out" && movement.createdAt?.startsWith(currentMonth)).length
  };
}
...
export async function deleteInventory(id) {
  ...
  await del(paths);
  return { deletedMovements: summarizeDeletedInventoryMovements(movements) };
}
